<template>
  <div class="basket-dropdown" v-if="basketWishList.length > 0">
    <div class="item" v-for="(item, index) in basketWishList" :key="index">
...
export default {
  ...
  methods: {
    removeItem(index) {
      this.basketWishList.splice(index, 1)
    ...
    toFindDuplicates() {
      ...
      duplicates.length > 0 ? this.$store.state.itemCount++ : []
    }
  }
}
</script>
